            ...
            for (int i = 0; i < _objects.Count; i++)
            {
                ...
                if (bullet.activeSelf)
                {
                    ...
                    if (Physics.Raycast(bullet.transform.position, tracker.Direction, out var hit, distance, HitLayerMask, QueryTriggerInteraction.Ignore))
                    {
                        ...
                        //if (hit.transform.gameObject.tag == "Enemy") //Checks collision object to see if it's an enemy
                        //{
                        ...
                        //BloodSquirt ScriptFromObj = NewBloodSpurt.GetComponent<BloodSquirt>();                            //These two are also from forums, bad for numerous reasons
                        //ScriptFromObj.MakeBloodSquirt(hit.transform.tag);

                        //var scriptInstance = Instantiate(BloodSpurtObj).GetComponent(BloodSquirt);                        //These two are from the forums, still says not in context
                        ...
                        if (hit.transform.gameObject.GetComponent<BodyPartId>().Is02Weak)
                        {
                            ThisDamage = BulletDamage * 2;
                        }
                        if (hit.transform.gameObject.GetComponent<BodyPartId>().Is03Mild)
                        {
                            ThisDamage = BulletDamage * 3;
                        }
                        if (hit.transform.gameObject.GetComponent<BodyPartId>().Is04Normal)
                        {
                            ThisDamage = BulletDamage * 4;
                        }
                        if (hit.transform.gameObject.GetComponent<BodyPartId>().Is05Rough)
                        {
                            ThisDamage = BulletDamage * 5;
                        }
                        if (hit.transform.gameObject.GetComponent<BodyPartId>().Is06Strong)
                        {
                            ThisDamage = BulletDamage * 6;
                        }
                        if (hit.transform.gameObject.GetComponent<BodyPartId>().Is07VeryStrong)
                        {
                            ThisDamage = BulletDamage * 100;
                        }

                        if (hit.transform.gameObject.GetComponent<BodyPartId>().EnemyStatsObj.GetComponent<EnemyStats>().EnemyArmor > 0)
                        {
                            TempDamage = ThisDamage;
                            ThisDamage -= hit.transform.gameObject.GetComponent<BodyPartId>().EnemyStatsObj.GetComponent<EnemyStats>().EnemyArmor;
                            hit.transform.gameObject.GetComponent<BodyPartId>().EnemyStatsObj.GetComponent<EnemyStats>().EnemyArmor -= TempDamage;
                        }
                        hit.transform.gameObject.GetComponent<BodyPartId>().EnemyStatsObj.GetComponent<EnemyStats>().EnemyHealth -= ThisDamage;
                    //*/
                        //}

                        if (hit.transform.gameObject.tag == "Hat")
                        {
                            hit.transform.gameObject.GetComponent<Rigidbody>().constraints = RigidbodyConstraints.None;
                            hit.transform.transform.parent = null;
                            Destroy(hit.transform.gameObject);

                            GameObject HatInst = Instantiate(HatObj, hit.point, this.transform.rotation);
                        }
